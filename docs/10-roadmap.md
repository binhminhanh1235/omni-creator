...
**Status: DONE / VERIFIED** via PR #89, exact-head CI #429 / run `34204676076`, guarded squash merge `37f165dd59eef51bbf51b34fd814ccf5fff0b37c`, and post-merge CI #430 / run `34205172307`.
...
- automatic, manual and external source artifacts use the same recovery path; recovery does not create provider success or a shadow database/state machine
- replacement invalidates the existing dependency cone, then canonical ProductionPack is rebuilt only from verified selected inputs
- Resolve/DaVinci export is regenerated by the existing Phase 9 exporter from the rebuilt ProductionPack
- durable recovery state contains logical URIs only; Data Root move/rebind regenerates path-bearing interchange output for the active binding
- desktop exposes Repair / Relink Visual, Audio, Timing, Audio + Timing and Rebuild + Regenerate Resolve; read-only mode remains inspection-only
