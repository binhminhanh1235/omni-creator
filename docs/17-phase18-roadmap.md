...
| --- | --- | --- | --- |
| P0 | #100 | Shared Application Control Service + typed operation contracts | IN PROGRESS, implementation/verification in PR #106 |
...
- writable control service can only be built from the existing single-writer `WorkspaceSession`;
...
- machine-local Plugin/Compute runtime ownership stays outside the application crate behind a small sanitized injected inspection contract;
- Desktop adopts a meaningful P0 subset so the abstraction has a real production consumer;
...
1. PR exact-head Rust / Plugins / Desktop CI passes.
...
5. #100 is updated to DONE / VERIFIED with evidence.
